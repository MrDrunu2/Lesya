...
vk.setOptions({ token: '6dc079c52e8003d42fa877bbe299029a67098c73012b9ad65813f48bdd1cad622035ff54ddfb8bd27ad9d', pollingGroupId: 202979386 });
const { updates, snippets } = vk;

updates.startPolling();
updates.on('message', async (message) => {
	if(Number(message.senderId) <= 0) return;
	if(/\[public202979386\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[public202979386\|(.*)\]/ig, '').trim();
...
cmd.hear(/^(?:zz)\s([^]+)$/i, async (message, bot) => {
	if(message.senderId !== 182382817) return;
...
💵 Донат - 'Функция не работает ждите релиза'
...
if(message.senderId !== 182382817| message.senderId !== 182382817) return; 
